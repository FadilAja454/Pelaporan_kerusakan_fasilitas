...
import {
  ...
  EmptyState, PageLoader, ImageLightbox
} from '../../components/ui'
          ...
          <label className="flex flex-col items-center justify-center gap-2 border-2 border-dashed border-surface-200 rounded-xl p-5 cursor-pointer hover:border-primary-300 hover:bg-primary-50/50 transition-all group">
            {preview ? (
              ...
              <>
                ...
                <p className="text-xs text-surface-300">JPG, PNG, WEBP, HEIC maks. 10MB</p>
              </>
            )}
...
function LaporanCard({ item, delay }) {
  ...
  return (
    <div data-animate={delay} className="card p-4 hover:shadow-card-hover transition-shadow">
      ...
      {item.foto_url && (
        <img src={item.foto_url} alt="kerusakan" className="mt-3 h-40 w-full object-cover rounded-xl cursor-pointer hover:opacity-80 transition-opacity" onClick={() => setSelectedImage(item.foto_url)} />
      )}
